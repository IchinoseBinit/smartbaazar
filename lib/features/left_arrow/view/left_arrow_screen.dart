...
// State Provider for direction (true = forward, false = reverse)
final isForwardProvider = StateProvider<bool>((ref) => true);
...
class LeftArrowScreen extends ConsumerStatefulWidget {
  ...
  const LeftArrowScreen({super.key, this.scrollController});
...
// Default height for first tab
  ...
  void initState() {
    items = [
      left_arrow(context, const Color(0xff362664), '999'),
      left_arrow(context, const Color(0xff901B41), '1300'),
      left_arrow(context, const Color(0xff362664), '3400'),
    ];
    ...
    //   },
    // );
    // Default selected index to 3 (LeftArrowScreen)
    // selectedIndex = 3;
  ...
  Widget build(BuildContext context) {
    ...
    final isForward = ref.watch(isForwardProvider);
    ...
    void onPageChanged(int index, CarouselPageChangedReason reason) {
      final isForward = ref.read(isForwardProvider);
      if (index == items.length - 1) {
        ref.read(isForwardProvider.notifier).state = false; // Reverse
      } else if (index == 0) {
        ref.read(isForwardProvider.notifier).state = true; // Forward
      }
      ref.read(currentIndexProvider.notifier).state = index;
    }

    void goToNextOrPrevious() {
      final isForward = ref.read(isForwardProvider);
      if (isForward) {
...
Container left_arrow(BuildContext context, Color color, String amount) {
  return Container(
    width: double.infinity,
    decoration: BoxDecoration(
      ...
      children: [
        ...
        SizedBox(
          ...
          child: ElevatedButton(
              ...
              onPressed: () {
                makepaymentnow(context, amount, false);
              },
              child: Text(
                "Choose Plan",
                style: headerstyle.copyWith(
